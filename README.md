...
"작업 시작하기 전에 무조건 업데이트부터 받자!" (이거 안 하면 나중에 코드 다 꼬임!) 다른 팀원이 깃허브에 올려둔 최신 코드를 내 컴퓨터로 가져오는 과정이야.

왼쪽 아래 확인: 커서 화면 왼쪽 맨 아래에 🔄 (화살표가 도는 모양) 아이콘이 있을 거야. (또는 Sync 글자)

클릭: 그걸 누르면 "This action will pull and push commits..." 라고 뜨는데 OK 누르면 돼.
