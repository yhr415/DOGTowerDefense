...
유기견 타워 디펜스는 뉴스로만 소비되던 유기견 문제를 ‘행동과 돌봄’의 이야기로 바꾸고자 합니다.
</p>
...
즉, 유기견 타워 디펜스는 " 정해진 경로로 이동하는 버림받고 상처받은 강아지들의 치유와 회복을 위해 다양한 돌봄 타워를 배치하고 업그레이드하는 게임 "이라고 정의할 수 있다.
...
### API 정보
![유기동물 api](images/readme_img_2_1.png)
- 대전 유기동물공고 오픈 API 연결 : https://www.daejeon.go.kr/ani/index.do 
...
아울러, 실제 유기견 입양 절차 및 관련 정보를 확인할 수 있는 QR 코드를 삽입하여, 게임 외부의 행동으로 이어질 수 있는 상호작용을 강화하였다.
  ...
  <sub>
    <b>4. Future Improvements & Expansion</b>
  </sub>

현재 본 게임은 각 스테이지 종료 시 '대전 유기동물공고 API'를 활용하여, 실제 입양 가능한 유기동물의 정보를 제공하는 구조로 설계되어 있다. 

향후 발전 방향으로는, 단일 API에 국한되지 않고 다양한 유기동물 보호 및 입양 관련 플랫폼의 API를 연동하여 정보의 범위와 정확성을 확장할 계획이다. 예를 들어, 지역별 보호소 데이터, 입양 후기, 보호 현황 등을 제공하는 외부 서비스와의 연계를 통해, 게임 이용자가 보다 폭넓은 정보를 접할 수 있도록 할 수 있다.

또한, 여러 API로부터 수집된 데이터를 통합·가공하여 지역, 동물 유형, 입양 가능 여부 등 조건에 따른 맞춤형 정보 제공이 가능하도록 시스템을 고도화할 수 있다. 이를 통해 스테이지 진행과 실제 유기동물 정보 간의 연결성을 더욱 강화하고, 게임 내 서사와 사회적 메시지를 한층 확장할 수 있을 것으로 기대한다.
